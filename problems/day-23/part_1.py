#!/usr/bin/env pypy3
import functools
import sys

pos = "DC" "CA" "AB" "BD" "..........."

adj = [
    [1],  # 0
    [0, 10],  # 1
    [3],  # 2
    [2, 12],  # 3
    [5],  # 4
    [4, 14],  # 5
    [7],  # 6
    [6, 16],  # 7
    [9],  # 8
    [8, 10],  # 9
    [9, 11, 1],  # 10
    [10, 12],  # 11
    [11, 13, 3],  # 12
    [12, 14],  # 13
    [13, 15, 5],  # 14
    [14, 16],  # 15
    [15, 17, 7],  # 16
    [16, 18],  # 17
    [17],  # 18
]

answer = "AA" "BB" "CC" "DD" "..........."


@functools.lru_cache(maxsize=None)
def solve(pos):
    result = float("inf")

    if pos == answer:
        return 0
    for i, c in enumerate(pos):
        if c == ".":
            continue

        if i < 2:
            if all(p == "A" for p in pos[0 : i + 1]):
                continue
        elif i < 4:
            if all(p == "B" for p in pos[2 : i + 1]):
                continue
        elif i < 6:
            if all(p == "C" for p in pos[4 : i + 1]):
                continue
        elif i < 8:
            if all(p == "D" for p in pos[6 : i + 1]):
                continue

        dist = {}

        def visit(i, cost):
            if i in dist:
                return
            if pos[i] != ".":
                return
            dist[i] = cost
            for a in adj[i]:
                visit(a, cost + 1)

        for a in adj[i]:
            visit(a, 1)

        for a, d in dist.items():
            if a in (i, 10, 12, 14, 16):
                continue
            if a < 2:
                if c != "A" or any(p != "A" for p in pos[0:a]):
                    continue
            elif a < 4:
                if c != "B" or any(p != "B" for p in pos[2:a]):
                    continue
            elif a < 6:
                if c != "C" or any(p != "C" for p in pos[4:a]):
                    continue
            elif a < 8:
                if c != "D" or any(p != "D" for p in pos[6:a]):
                    continue
            elif 8 <= i:
                continue

            new_pos = "".join(
                c if j == a else "." if j == i else p for j, p in enumerate(pos)
            )
            cost = d * (
                1 if c == "A" else 10 if c == "B" else 100 if c == "C" else 1000
            )

            result = min(result, cost + solve(new_pos))

    return result


print(solve(pos))
